...
class RawSignalVisualizer(tk.Frame):
    ...
    def init_listbox_signals(self):
        ...
        scrollbar = ttk.Scrollbar(
            self.frame_listbox, orient=tk.VERTICAL, command=self.listbox_objects.yview
        )
    ...
    def init_listbox_channels_frame(self):
        ...
        scrollbar = ttk.Scrollbar(
            frame_listbox, orient=tk.VERTICAL, command=self.listbox_channels.yview
        )
    ...
    def channel_selected(self, event):
        ...
        try:
            ...
            if len(selected_channel_idxs) == 1:
                ...
                self.selected_channel = self.selected_signal.signal[
    ...
    def plot_selected_signal(self):
        ...
        self.canvas.figure.suptitle(
            "Idx: {}, Class: {}, Channel: {}".format(obj_idx, obj_class, channel_idx)
        )
        ...
        f, t, Sxx = signal.spectrogram(
            self.selected_channel, self.raw_signals.sample_rate
        )
        ...
        ax2.magnitude_spectrum(
            self.selected_channel,
            Fs=self.raw_signals.sample_rate,
            scale="dB",
            color="C1",
        )
    ...
    def plot_frame_init(self):
        ...
        self.canvas = FigureCanvasTkAgg(
            self.figure, master=frame_listbox
        )  # A tk.DrawingArea.
        self.canvas.draw()
